...
def well_imputed_by_maf(inWell_imputed, outWell_imputed, dataset=''):
    """
    ...
    outWell_imputed_out.writelines('\\t'.join(['DATASET', 'MAF<1%', 'MAF 1-5%', 'MAF>5%']) + '\\n')
    ...
    for line in info_datas:
        ...
        if "SNP" in line and "Rsq" in line:
            ...
            if maf >= 0.5:
                maf = 1 - maf
            if maf < 0.01:
                datas[chrm]['rare'].append(maf)
                total['rare'].append(maf)
                datas[chrm]['total'] += 1
            elif maf > 0.05:
                datas[chrm]['common'].append(maf)
                total['common'].append(maf)
                datas[chrm]['total'] += 1
            elif maf <= 0.05 and maf >= 0.01:
                datas[chrm]['moderate'].append(maf)
                total['moderate'].append(maf)
                datas[chrm]['total'] += 1
    try:
    ...
    # Write per chromosome or pop
    for chrm in datasets:
        tot = datas[chrm]['total']
        rare = len(datas[chrm]['rare'])
        moderate = len(datas[chrm]['moderate'])
        common = len(datas[chrm]['common'])
        if rare >= 1000000:
        ...
        if tot == 0:
            outWell_imputed_out.write("dataset {} is empty (tot=0)".format(chrm))
            outWell_imputed_out_1.write("dataset {} is empty (tot=0)".format(chrm))
        else:
            outWell_imputed_out_1.writelines('\\t'.join([chrm, \
                                                         rare_ + str(rare * 100 / tot) + '%)', \
                                                         moderate_ + str(moderate * 100 / tot) + '%)', \
                                                         common_ + str(common * 100 / tot) + '%)', \
                                                         str(format(tot / 1000000., '0,.1f')) + 'M']) + '\\n')
    # Write the overall for dataset
    outWell_imputed_out.writelines('\\t'.join([dataset, \
                                               str(len(total['rare'])), \
                                               str(len(total['moderate'])), \
                                               str(len(total['common']))]) + '\\n')
...
args.dataset = "${target_name}"
if args.inWell_imputed and args.outWell_imputed:
    well_imputed_by_maf(args.inWell_imputed, args.outWell_imputed, args.dataset)
