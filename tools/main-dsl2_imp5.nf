...
include { impute5_phasing_eagle } from './modules/phasing'
...
workflow preprocess {
    ...
    main:
        ...
        params.chromosomes.split(',').each{ chrm ->
            impute5_genetic_map = sprintf(params.impute5_genetic_map, chrm)
            check_files([ impute5_genetic_map ])
...
workflow phasing{
    ...
    main:
        impute5_phasing_eagle(data)

    emit: 
        chunks_phased = impute5_phasing_eagle.out
}
...
workflow {
    ...
    phasing_data = Channel.from(params.ref_panels)
        ...
        .flatMap{ ref_name, ref_imp5, ref_vcf, dataset, chrm, start, end, dataset_, dataset_vcf ->
            vcf = sprintf(ref_vcf, chrm)
            imp5 = sprintf(ref_imp5, chrm)
            imp5_idx = sprintf("${ref_imp5}.idx", chrm)
            if(vcf.endsWith("vcf.gz")){
                vcf_idx = "${vcf}.tbi" 
            }
            else if(vcf.endsWith("bcf")){
            ...
            return [ [ chrm, ref_name, file(imp5), file(imp5_idx), file(vcf), file(vcf_idx), file(params.eagle_genetic_map), start, end, dataset, dataset, file(dataset_vcf)] ]
        }
    phasing(phasing_data)
   
    //// Imputation
    impute_data = phasing.out.chunks_phased
                .map{ chrm, start, end, dataset_name, dataset_vcf, dataset_vcf_tbi, ref_name, ref_vcf, ref_imp5, ref_imp5_idx, dataset_name1 -> [ chrm, start, end, dataset_name, file(dataset_vcf), file(dataset_vcf_tbi), ref_name, file(ref_vcf), file(ref_imp5), file(ref_imp5_idx), dataset_name1, file(sprintf(params.impute5_genetic_map, chrm)) ] }
