...
for (chrm in chromosomes){
    ...
    legend_map << [chrm, ref_1_legendFile, ref_2_legendFile, params.ref_1.name, params.ref_2.name]
}

process generate_chunks {
    tag "chunks_${ref_1_name}_${ref_2_name}_${chrm}"
    memory { 10.GB * task.attempt }
    time { 5.h * task.attempt }
    publishDir "${params.output_dir}/chunks", overwrite: true, mode:'copy'

    input:
        set val(chrm), file(ref_1_legendFile), file(ref_2_legendFile), val(ref_1_name), val(ref_2_name) from legend_map
    output:
        set val(chrm), file(chunkFile) into generate_chunks
    script:
        chunk_size = params.chunk_size[0..-7]
        chunkFile = "${ref_1_name}_${ref_2_name}_analysis_chunks_${chunk_size}Mb_chr${chrm}.txt"
        """
        gunzip -c ${ref_1_legendFile} > ${ref_1_legendFile.baseName}
        gunzip -c ${ref_2_legendFile} > ${ref_2_legendFile.baseName}
        /usr/local/bin/python2.7 ${params.homedir}/chipimputation/scripts/generate_chunks_2refs.py \
            --legend_file_list '${ref_1_legendFile.baseName},${ref_2_legendFile.baseName}' \
            --outfile ${chunkFile} \
            --chunk_size ${params.chunk_size} \
            --chrm ${chrm}
        rm -f ${ref_1_legendFile.baseName} ${ref_2_legendFile.baseName}
        """
}
...
chunks_datas = []
def transform_chunk = { chrm, chunk_file ->
    ...
    chunks.each{ chunk_data ->
        ...
        if (chrm ==chrm_){
            ...
            chunks_datas << [chrm, chunk_start, chunk_end, ref_1_hapFile, ref_1_legendFile, ref_1_mapFile, ref_1_sampleFile, ref_2_hapFile, ref_2_legendFile, ref_2_mapFile, ref_2_sampleFile]

        }
    }
    return chunks_datas
}
cross_refs_data = generate_chunks_1
...
//    return res
//}
//def combine_chunk_data_with_2refs = { chrm, chunk_start, chunk_end ->
//    res = []
//    ref_1_hapFile = file(sprintf(params.ref_1.hapFile, chrm))
//    ref_1_legendFile = file(sprintf(params.ref_1.legendFile, chrm))
//    ref_1_mapFile = file(sprintf(params.ref_1.mapFile, chrm))
...
process cross_impute_2refs {
    ...
    publishDir "${params.impute_result}/cross_imputed_${params.ref_1.name}_${params.ref_2.name}/${chromosome}", overwrite: true, mode: 'copy'
    input:
        set val(chromosome), val(chunkStart), val(chunkEnd), file(ref_1_hapFile), file(ref_1_legendFile), file(ref_1_mapFile), file(ref_1_sampleFile), file(ref_2_hapFile), file(ref_2_legendFile), file(ref_1_mapFile), file(ref_2_sampleFile) from cross_refs_data
    output:
    ...
    shell:
        outfile = "${params.ref_1.name}_${params.ref_2.name}_chr${chromosome}_${chunkStart}-${chunkEnd}_x-imputed.impute2"
